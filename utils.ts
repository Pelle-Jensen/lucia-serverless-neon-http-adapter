...
const createPreparedStatementHelper = (placeholder: (index: number) => string) => {
	const helper = (
...
export type DatabaseSession = Omit<SessionSchema, 'active_expires' | 'idle_expires'> & {
	active_expires: bigint;
	idle_expires: bigint;
};

export const transformDatabaseSession = (session: DatabaseSession): SessionSchema => {
	return {
		...session,
